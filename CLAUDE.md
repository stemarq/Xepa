...
**Com widget, a lista de instituições não existe — e isso é desenho.** `GET /grana/open-finance/instituicoes` devolve `[]` e `escolhaNoWidget: true` quando o provedor é agregador, e o app mostra um botão único. O widget já é o seletor, com busca e marca, sobre o catálogo inteiro; a nossa lista era um segundo seletor para a mesma escolha, sem filtro nenhum — tolerável com os 6 bancos do simulador, um paredão com os 233 reais. Consequência: o nome do banco só se conhece na autorização, então o consentimento nasce como "Conectando…" e `autorizarConsentimento` o corrige junto com o `id_externo`.

**`includeSandbox` do widget segue o servidor.** Fixo em `true` no app, o widget listaria bancos de teste mesmo com a integração real ligada — e a lista é justamente o que diz ao usuário se ele está conectando algo de verdade.
